 ...
 * @author Aruna Csw
 */
public class ApproveDailyJobServlet extends HttpServlet {
...
if (    null != action) switch (action) {
            case "Approve":
                Statement stm;
        String Sql="UPDATE `advancement_payment` SET `status`='Approved' WHERE `ap_id`='"+ap_id+"'" ;
        try {
            ...
            case "Reject":
Statement stm1;
        String Sql1="UPDATE `advancement_payment` SET `status`='Rejected' WHERE `ap_id`='"+ap_id+"'" ;
        try {
