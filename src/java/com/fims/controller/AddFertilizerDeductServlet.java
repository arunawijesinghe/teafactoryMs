 ...
 * @author Aruna Csw
 */
public class AddFertilizerDeductServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String sysid = request.getParameter("fcolid");
            String date = request.getParameter("date1");
            String fertiIssuedId = request.getParameter("f_sent_id");
            double totWeight = Double.parseDouble(request.getParameter("tot_f_weight"));
            double monfrate = Double.parseDouble(request.getParameter("mon_f_rate"));
            double price = Double.parseDouble(request.getParameter("fert_price"));
            SimpleDateFormat df = new SimpleDateFormat("MM/dd/yyyy");
            
            Date sdate =df.parse(date);
            FertilizerDedution fDeduct = new FertilizerDedution();
            
            fDeduct.setSystemId(sysid);
            fDeduct.setDate(sdate);
            fDeduct.setFertiIssuedId(fertiIssuedId);
            fDeduct.setTotFertiWeight(totWeight);
            fDeduct.setMonthlyRate(monfrate);
            fDeduct.setPrice(price);
            
            DeductionService.fertiDeductService(fDeduct);
